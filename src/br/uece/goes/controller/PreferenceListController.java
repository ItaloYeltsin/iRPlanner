...
public class PreferenceListController {
	// Preference items
	public static ObservableList<Preference> preference = FXCollections
			.observableArrayList();
	...
	public PreferenceListController(ListView<Preference> prefList,
			MenuButton newPref) {
	...
	private void initialize() {
		...
		Image imageAddPreff = new Image(
				Main.class.getResourceAsStream("images/add.png"));
	...
	public static class XCell extends ListCell<Preference> {
		...
		Image imageEdit = new Image(
				Main.class.getResourceAsStream("images/edit.png"));
		Image imageSatisfied = new Image(
				Main.class.getResourceAsStream("images/satisfied.png"));
		Image imageNotSatisfied = new Image(
				Main.class.getResourceAsStream("images/notsatisfied.png"));

		Preference lastItem;

		static public Solution solution = null;
		...
		public XCell() {
			...
			hbox.getChildren().addAll(satisfiedButton, label, pane, editButton,
					deleteButton);
		...
		protected void updateItem(Preference item, boolean empty) {
			...
			if (empty) {
				...
				if (solution != null) {
					try {
						if (item.evaluate(solution) == 0.0) {
							satisfiedButton.setGraphic(new ImageView(
									imageNotSatisfied));
						} else {
							satisfiedButton.setGraphic(new ImageView(
									imageSatisfied));
			...
			editButton.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent event) {
					...
					slider.addEventFilter(MouseEvent.MOUSE_RELEASED,
							new EventHandler<MouseEvent>() {
								...
								public void handle(MouseEvent event) {
									...
									item.setWeight((int) Math.round(slider
											.getValue()));
									isSliderActived = false;
								}
							});
